...
- Run `uv run scripts/generate_theme.py` to choose a scheme interactively.
- Run `make theme` to choose a scheme and reload Hyprland, Waybar, and SwayNC.
- Use `uv run scripts/generate_theme.py --scheme <name>` for non-interactive selection and `--list` to see names.
- Run `uv run scripts/generate_theme.py --check` to detect stale generated files.
...
- Run `uv run scripts/generate_theme.py --check` after theme edits.
